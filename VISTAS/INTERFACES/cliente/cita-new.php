...
<?php
    ...
    $res = $conexion->query("SELECT * from usuario where Rol = 'Cliente' and NumeroDoc = $num_doc_session")->fetchall();
    $res1 = $conexion->query("SELECT * from usuario where Rol = 'Odontologo' or Rol = 'Administrador'")->fetchall();
    $res2 = $conexion->query("SELECT * from agenda where Estado = '1'")->fetchall();

	$res4 = $conexion->query("SELECT Agenda.id_agenda, Cita_medica.Fecha, Cita_medica.Hora from agenda inner join 
	cita_medica on Agenda.id_agenda = Cita_medica.id_agenda  order by Agenda.id_agenda;")->fetchall();
?>
